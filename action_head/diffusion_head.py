...
class SinusoidalTimeEmbedding(nn.Module):
    def __init__(self, dim: int):
        super().__init__()
        self.dim  = dim
        self.proj = nn.Sequential(       # learned projection — standard in DDPM
            nn.Linear(dim, dim * 4),
            nn.SiLU(),
            nn.Linear(dim * 4, dim),
        )
...
class ActionDenoiseModel(nn.Module):
    def __init__(self, cfg: DiffusionConfig, time_emb_dim=64, hidden_dim=512):
        ...
        # project condition separately before concat
        self.cond_proj = nn.Sequential(
            nn.Linear(cfg.cond_dim, hidden_dim),
            nn.SiLU(),
        )

        chunk_dim = cfg.action_dim * cfg.action_horizon
        in_dim    = chunk_dim + time_emb_dim + hidden_dim
    ...
    def forward(self, x_t, t, cond):
        ...
        x        = torch.cat([x_t, t_emb, cond_emb], dim=-1)
        return self.net(x)
...
class DiffusionHead(nn.Module):
    ...
    def sample(self, cond):
        ...
        for t_step in reversed(range(self.cfg.T)):
            ...
            x0_pred = x0_pred.clamp(-1, 1)   # stabilize reverse diffusion
